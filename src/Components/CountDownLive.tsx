...
const DEFAULT_DIGIT_TXT_STYLE = { color: '#fff' };
const DEFAULT_TIME_LABEL_STYLE = { color: '#fff' };
const DEFAULT_SEPARATOR_STYLE = { color: '#fff' };
...
class CountDownLive extends React.Component {
  static propTypes = {
    id: PropTypes.string,
  ...
  renderCountDown = () => {
    ...
    return (
      ...
      >
        {timeToShow.includes('D') ? this.renderDoubleDigits(timeLabels.d, [days]) : null}
        {showSeparator && timeToShow.includes('D') && timeToShow.includes('H') ? this.renderSeparator() : null}
        {timeToShow.includes('H') ? this.renderDoubleDigits(timeLabels.h, [hours]) : null}
        {showSeparator && timeToShow.includes('H') && timeToShow.includes('M') ? this.renderSeparator() : null}
        {timeToShow.includes('M') ? this.renderDoubleDigits(timeLabels.m, [minutes]) : null}
        {showSeparator && timeToShow.includes('M') && timeToShow.includes('S') ? this.renderSeparator() : null}
        {timeToShow.includes('S') ? this.renderDoubleDigits(timeLabels.s, [seconds]) : null}
      </Component>
    );
  };
...
export default CountDownLive;
export { CountDownLive };